...
powerful and tasks are often very quickly accomplished with `R` when comparing 
to its competitors, and is therefore the tool of choice for many statisticians,
data scientists, and the like.

A common argument against it, however, is the lack of e.g. type safety, and 
a compiler to ensure that the code is correct. To make the situation even 
more dangerous, it is very common in `R` that functions accept different
types for the same argument, and they may even return different types depending
on various circumstances. The very large community, which is one `R`'s strengths,
can also dangerous when developers adhere to different principles, styles,
and standards.

When putting an `R` program into prodcution one should therefore be aware of
the various sources of risk. External data sources or schematics may change; 
...
and take proper action. In particular, unexpected bahavior may not 
itself raise an error but may result in corrupted data which may propergate,
and when errors occur down the road the initial source of trouble can be
...
The first takes a value (`value.`) and a set of conditions (`...`) which are
verifyied for the value. Upon success, the value itself is returned and upon
failure an error is raised (default behaviour, which can be modified.)

Stating the conditions is simple, each are expressions which result in 
`TRUE` or `FALSE` and are separated with commas (technical note: conditions
are checked with `isTRUE`, and conditions therefore not result in vectors
as this will count as failure.) To reference the value itself, use the 
dot-placeholder (`.`). 

As an example, suppose you have a function `get_matrix` which is supposed to
return a square matrix. The funcion may read an external text file, or 
...
a character somehow snuck its way in, coercing everything to the character type.
In this case the default behaviour is to raise an error with details on which
...
The second function, `ensures_that`, is ideal for creating reusable contracts,
...
Here is how the first example is made without the pipe operator, `%>%`:
...
# Controlling failure
The main purpose of the `ensurer` is to esure that values are as they are
expected, and if not alert the stakeholder(s) immediately. The default
behaviour is therefore to raise an error. There could be several reasons 
...
The `ensurer` has a simple mechanism for changing the behaviour: 
specify the named argument `fail_with` in the call to `ensure_that` or 
`ensures_that`. It is possible to pass in a value which will be returned
(e.g. an empty `data.frame` with the correct columns, or maybe even just
`NA`). More often, however, more dynamic behaviour is needed, and for 
